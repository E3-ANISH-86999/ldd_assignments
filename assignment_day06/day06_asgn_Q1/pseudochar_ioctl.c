...
static long pchar_ioctl(struct file *pfile, unsigned int cmd, unsigned long param){
	...
	switch (cmd){
		...
		case FIFO_RESIZE:
			...
			nbytes = kfifo_out(&mybuf, temp, len);
...
static int __init pchar_init(void){
	...
	major = MAJOR(devno);
	printk(KERN_INFO "%s: alloc_chrdev_region() device number: %d.\n", THIS_MODULE->name, major);
    // create device class
	...
	printk(KERN_INFO "%s: class_create() created pchar device.\n", THIS_MODULE->name);
	// create device file
...
static void __exit pchar_exit(void){
	...
	unregister_chrdev_region(devno, 1);
	printk(KERN_INFO "%s: unregister_chrdev_region() released device number: %d.\n", THIS_MODULE->name, major);
